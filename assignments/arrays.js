...
const challenge1 = (carInventory, id) => {
...
const lastCar = (carInventory) => {
    return carInventory[carInventory.length - 1].car_make + ", " + carInventory[carInventory.length - 1].car_model;
}
...
const sortModels = (carInventory) => {
    ...
    for (let i = 0; i < carInventory.length; i++) {
      carModels.push(carInventory[i].car_model);
      }
      carModels.sort();
      return carModels.toString();
  }
...
let BMWAndAudi = [];
const carSearch = (carInventory, type1, type2) => {
  for (let i = 0; i < carInventory.length; i++) {
    if (carInventory[i].car_make === type1) {
      BMWAndAudi.push(carInventory[i]);
    }
    else if (carInventory[i].car_make === type2) {
      BMWAndAudi.push(carInventory[i]);
    }
  }
  return JSON.stringify(BMWAndAudi);
}
console.log(carSearch(inventory, "BMW", "Audi"));
